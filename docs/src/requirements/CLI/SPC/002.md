...
- uuid: 0ac3ee79-3ee1-4f4b-ac07-7523f99f5383
  ...
  hrid: CLI-SYS-017
---
# CLI-SPC-002 Requirement Listing CLI Experience
...
- Default limit is 200 results
- `--limit N` where N > 0 sets a custom page size
- `--limit 0` disables pagination entirely (shows all results, no footer)
  - Useful for scripting and processing full datasets
  - Follows common CLI convention (e.g., `head`, `tail`, database tools)
- `--offset N` skips first N results (works with or without limit)
- When unlimited (`--limit 0`), header shows `limit: unlimited` instead of `limit: 0`
