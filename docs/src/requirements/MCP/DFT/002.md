...
## Evidence
- `crates/req-mcp/src/state.rs` wraps a `Directory` loaded only in `ServerState::new`.
- `crates/req-mcp/src/main.rs` initializes state once and never watches or refreshes it.
